#!/usr/bin/env python3
"""
Regenerate all 331 HVAC problem HTML files with PROPERLY DISTRIBUTED office hours coverage.
Uses oh_lookup_v2.json to map OH content to correct problems.
"""
...
def build_oh_html(oh_list, problem_num):
    """Build OH HTML for a specific problem using correct Q&A pairs."""
    if not oh_list:
...
'''
    ...
    for oh_entry in oh_list:
        session = oh_entry.get('session', 'Unknown')
        question = oh_entry.get('question', '')
        answer = oh_entry.get('answer', '')
        
        oh_html += f'''            <div class='qa-group'>
                <div class='qa-session'>{session}</div>
                <div class='qa-question'><strong>Q:</strong> {question}</div>
                <div class='qa-answer'><strong>A:</strong> {answer}</div>
            </div>
'''
...
def main():
    print("🔄 REGENERATING ALL 331 HVAC PROBLEMS WITH CORRECT OH DISTRIBUTION\n")
    print("=" * 70 + "\n")
    ...
    for chapter_name, (section_code, count) in HVAC_CHAPTERS.items():
        ...
        for problem_num in range(1, count + 1):
            ...
            # Look up OH coverage for this specific problem
            oh_key = f"HVAC_{chapter_name.replace('-', '_')}_{problem_num}"
            oh_list = oh_lookup.get(oh_key, [])
            oh_html = build_oh_html(oh_list, problem_num)
    ...
    print(f"OH Distributed: {oh_distributed} problems")
    print(f"Location:     {PROBLEMS_DIR}/")
    print(f"\nNext: git add -A && git commit -m 'Fix HVAC OH distribution' && git push")
