#!/usr/bin/env python3
"""
Generate all 331 HVAC problem HTML files with embedded PDFs.
...
def extract_office_hours(html_path):
    """Extract office hours section from existing HTML."""
    try:
        with open(html_path, 'r') as f:
            html = f.read()
        
        # Look for sidebar-right content
        match = re.search(r'<div class="sidebar-right">(.*?)</div>\s*</div>\s*<div class="footer">', html, re.DOTALL)
        if match:
            return match.group(1).strip()
        return '<div class="oh-section-title">Office Hours<span class="oh-count">0</span></div><div class="oh-subtitle">Student questions asked in live office hours about this problem</div><p style="color:#999;">No office hours coverage for this problem.</p>'
    except:
        return '<div class="oh-section-title">Office Hours<span class="oh-count">0</span></div><div class="oh-subtitle">Student questions asked in live office hours about this problem</div><p style="color:#999;">No office hours coverage for this problem.</p>'

def generate_html(chapter_name, problem_num, pdf_filename, video_content, oh_content):
...
def main():
    print("🚀 GENERATING ALL 331 HVAC PROBLEMS\n")
    print("=" * 70 + "\n")
    ...
    for chapter_name, (section_code, count) in HVAC_CHAPTERS.items():
        ...
        for problem_num in range(1, count + 1):
            ...
            existing_html_path = PROBLEMS_DIR / f"HVAC-{chapter_name}-{problem_num}.html"
            video_content = extract_video_synthesis(existing_html_path)
            oh_content = extract_office_hours(existing_html_path)
            
            # Generate HTML
            html_content = generate_html(chapter_name, problem_num, pdf_filename, video_content, oh_content)
