...
Properly extracts and preserves video synthesis and office hours sections.

Usage:
  python3 generate_all_tfs_pdfs_v3.py --dry-run    # Preview without writing
  python3 generate_all_tfs_pdfs_v3.py --confirm    # Generate all files
"""
...
# Load original content from recovery file
ORIGINAL_CONTENT = {}
original_content_file = Path(__file__).parent / 'original_content.json'
if original_content_file.exists():
    with open(original_content_file, 'r') as f:
...
def get_original_sections(problem_key):
    """Get video and OH sections from recovered original content."""
    if problem_key in ORIGINAL_CONTENT:
        return {
            'video': ORIGINAL_CONTENT[problem_key].get('video', ""),
            'oh': ORIGINAL_CONTENT[problem_key].get('oh', "")
        }
    return {'video': "", 'oh': ""}
...
def generate_html_for_problem(chapter_name, problem_num, pdf_filename, existing_sections=None):
    ...
    video_content = existing_sections.get('video', "")
    oh_content = existing_sections.get('oh', "")
    chapter_display = chapter_name.replace('#', '').strip()
    
    # Build video section (either with extracted content or placeholder)
    video_section = f'<div class="video-box">{video_content}</div>' if video_content else '<div class="video-box"><ul><li>Video synthesis will be populated here once available.</li></ul></div>'
    
    # Default OH section if not extracted
    if not oh_content:
        oh_content = '''<div class="sidebar-right">
...
def main():
    ...
    if not args.dry_run and not args.confirm:
        print("Usage:")
        print("  python3 generate_all_tfs_pdfs_v3.py --dry-run    # Preview without writing")
        print("  python3 generate_all_tfs_pdfs_v3.py --confirm    # Generate all 329 files")
        sys.exit(1)
    ...
    if args.dry_run:
        ...
        print(f"\n✅ Ready to proceed with: python3 generate_all_tfs_pdfs_v3.py --confirm")
        return
    ...
    if args.confirm:
        ...
        for chapter_name, info in TFS_MAPPING.items():
            ...
            for problem_num in range(1, count + 1):
                ...
                # Get original video/OH from recovered content
                existing_sections = get_original_sections(html_filename.replace('.html', ''))
